// File: src/components/SupplierOTPForm.tsx
...
const SupplierOTPForm: React.FC = () => {
  // Ambil email dari state hasil navigate dari register
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/user-supplier/verify-otp?email=${encodeURIComponent(email)}&otp=${code}`,
        { method: "POST" }
      );
      if (!response.ok) {
        let data = {};
...
export default SupplierOTPForm;
