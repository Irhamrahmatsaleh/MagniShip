// File: src/components/CustomerOTPForm.tsx
...
const CustomerOTPForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/user-customer/verify-otp?email=${encodeURIComponent(email)}&otp=${code}`,
        { method: "POST" }
      );
      if (!response.ok) {
        let data = {};
        ...
        throw new Error((data as any).message || "OTP salah atau sudah kadaluarsa.");
      }
      setSuccess(true);
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-green-100 via-blue-100 to-indigo-100 flex items-center justify-center">
      <form
