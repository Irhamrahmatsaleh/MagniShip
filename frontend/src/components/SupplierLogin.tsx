// File: src/components/SupplierLogin.tsx
...
const SupplierLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://localhost:8080/api/user-supplier/login", {
        method: "POST",
  ...
  return (
    <>
      ...
      <div className="min-h-screen w-full bg-gradient-to-br from-yellow-100 via-pink-100 to-indigo-200 flex items-center justify-center">
      <form
      ...
      >
        ...
        <button
          type="submit"
          className="bg-pink-600 text-white font-bold py-2 rounded-lg hover:bg-pink-700 transition mt-1"
          disabled={loading}
        >
          {loading ? "Masuk..." : "Masuk"}
        </button>
